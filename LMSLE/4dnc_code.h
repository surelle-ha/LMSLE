void layer1Decrypt(char *textfile, char *keyfile, char *option){
	/* Get Text File Start */ 
	...
	if(snprintf(buffer, sizeof(buffer), "document/%s.txt", file[0])>=sizeof(buffer)){
		...
		if(snprintf(buffer1, sizeof(buffer1), "key/%s.key", kfile[0])>=sizeof(buffer1)){
			...
			char * token = strtok(text, "Jkfk5p");
			int x = 0;
			char *layer[100];
			while( token != NULL ) {
				layer[x++] = token;
				token = strtok(NULL, "Jkfk5p");
			}
			if(strcmp(option, "-d")==0 || strcmp(option, "--display")==0){
				for(i = 0; (i < 100 && layer[i] != '\0'); i++){ /* Get root value */
					hexDec(layer[i], root, textfile, 1);
				}
				system("zenity --info --text=\"Decryption Done!\" --title=\"Status\"");
				...
				if(strcmp(dec,"y")==0 || strcmp(dec,"Y")==0){
					...
					for(i = 0; (i < 100 && layer[i] != '\0'); i++){ /* Get root value */
						hexDec(layer[i], root, textfile, 2);
					}
				}
				system("zenity --info --text=\"Decryption Done!\" --title=\"Status\"");
				main();
				...
				scanf(" %[^\n]", fname);
				for(i = 0; (i < 100 && layer[i] != '\0'); i++){ /* Get root value */
					hexDec(layer[i], root, fname, 3);
				}
				system("zenity --info --text=\"Decryption Done!\" --title=\"Status\"");
				main();
			}
			/* Decryption Process End here */
		}
		/* Get Key File End */
	}
	/* Get Text File End */
